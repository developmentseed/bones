...
Backbone.Controller.prototype.initialize = function(options) {
    ...
    // Add the controller to /controllers.js only if it's not a server-only controller.
    if (!(/\.server.plexus$/).test(this.constructor.__filename)) {
        options.controllerFiles.push(this.constructor.__filename);
    }
};
